/**
 * Icone Lucide (ISC) usate dal form, inline come SVG: niente emoji, niente font
 * di icone, niente richieste extra. Ogni voce è il contenuto interno del
 * `<svg viewBox="0 0 24 24">` a tratto 2 px. Aggiungere qui una nuova icona =
 * incollare i path da lucide.dev e usarla con `icona("nome")`.
 */
export const ICONE = {
  ...
  altro: '<circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/>',
  spunta: '<path d="M20 6 9 17l-5-5"/>',
  sinistra: '<path d="m12 19-7-7 7-7"/><path d="M19 12H5"/>',
  destra: '<path d="M5 12h14"/><path d="m12 5 7 7-7 7"/>',
  fotocamera:
  ...
  lucchetto: '<rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>',
  globo:
...
/** Markup SVG inline di un'icona (decorativa: aria-hidden). */
export function icona(nome: NomeIcona, classe = "icona"): string {
  return `<svg class="${classe}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${ICONE[nome]}</svg>`;
}
