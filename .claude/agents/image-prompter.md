...
- Output per ogni immagine: il FILE scaricato in `out/<slug>/img/` (gli URL BFL scadono: mai usarli come src) + `alt` in italiano; `images-trace.json` con `{model, aspect_ratio, prompt, seed}` per gate e checkpoint — `images.json` lo deriva l'editor alla conferma.
