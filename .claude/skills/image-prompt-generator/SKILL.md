...
## Procedura
1. **Style bible (una volta per sito):** parti dalla FOTOGRAFIA DEL PRESET (M9): leggi `site-renderer/presets/<preset>.meta.json` (il preset sta in `palette.json` → `brand.preset`) e usa `photographySpec` (soggetto/angolo/luce/grading/mood) + `fluxStyleFragment` (style/lighting/film_reference/mood/color_palette) come SPINA DORSALE dello style bible — ogni preset ha una fotografia sua, è un asse di varietà deliberato. Poi integra: palette hex del CLIENTE (non quella del frammento, che è dei neutri del preset), lente, **ambientazione italiana**. Riusa questa stringa in OGNI prompt → coerenza. Es.: «fotografia editoriale realistica, luce naturale morbida, tonalità coerenti con #1E3A8A e #F59E0B, 35mm, colori sobri, senza testo, interni e architettura residenziale italiana».
