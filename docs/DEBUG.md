...
| Chiave rifiutata in Impostazioni → Chiavi API (gruppo «Ottimizzazione del traffico») | il messaggio sotto il campo = `error` della POST `/api/setup/keys` (nessun log su disco, per scelta: il valore non deve mai finirci) | il messaggio dice già il servizio e cosa fare; testi e casi in `site-factory-editor/lib/chiavi-traffico.ts`, riprodotti senza rete da `node --experimental-strip-types scripts/test-chiavi.ts`. «non ha risposto entro N s» / «non raggiungibile» = rete o servizio, non la chiave. DataForSEO salva una metà senza prova finché manca l'altra: se la coppia viene rifiutata, controllare anche la metà già salvata. «valore troppo lungo per il portachiavi» = oltre 4000 caratteri (buffer di `security -i`); «salvataggio nel portachiavi incompleto» = rilettura diversa dal valore scritto. Presenza di una chiave senza leggerne il valore: `security find-generic-password -s site-factory -a <NOME>` (exit 0 = c'è); **mai** `-w` in un log o in una chat |
