...
1. **Il numero che cambia tutto: la compressione lato client.** Una foto da
   telefono di 2-8 MB, ridimensionata a 1600-2000 px sul lato lungo con qualità
   0,8, pesa 0,3-0,8 MB. Il renderer usa comunque immagini a 1600 px. Un lead con
   5-30 foto passa da 50-200 MB a **2-25 MB**; 130 lead al mese fanno 0,3-3 GB, non
   26. Con questi volumi il problema dello spazio quasi scompare, e la ripresa degli
   upload interrotti (tus e simili) diventa superflua: un file = una richiesta
   piccola e ripetibile.
2. **Destinazione raccomandata: Google Drive dell'agenzia, direttamente**, via il
   ...
   non conserva nulla.
3. **Nel browser: input nativo + un piccolo uploader proprio + compressorjs**
   (meno di 10 KB in totale). Niente FilePond o Uppy: pesano 50-100 KB, vanno
   ri-brandizzati, e FilePond ha un bug aperto dal 2023 che ricarica la pagina su
   iPhone con più di 10 foto, cioè esattamente il nostro caso.
...
- Contro: i byte passano dal VPS (n8n) invece di andare dritti allo storage; va
  ...
  verificare (Business Starter 30 GB per utente, Standard 2 TB), comunque
  sufficiente per anni ai volumi compressi se `_inbox` viene pulita.
...
|---|---|---|
| **Input nativo + uploader proprio (XHR con progresso, coda a 2 in parallelo, retry) + compressorjs** | ≈5-8 KB | **Scelto.** Controllo totale su brand, animazioni, memoria e accessibilità; 250-350 righe da scrivere e testare |
...
2. **`capture`**: mai sull'input principale (aprirebbe solo la fotocamera, uno
   ...
   `capture="environment"`.
3. **Compressione seriale, upload a 2 in parallelo** (3 al massimo): una decodifica
   da 12 MP occupa circa 48 MB di RGBA; i default di FilePond e Dropzone sono 2.
4. **Niente chunk né ripresa** sotto i 10 MB per file (docs tus: «non impostare
   chunkSize se non sei costretto»); un file = una richiesta idempotente.
...
│   ├── logo.<ext>
│   └── foto/01.jpg … 30.jpg     (già compresse, numerate = ordine scelto dal lead)
...
|---|---|---|
| Upload | Il form comprime, carica un file per richiesta con `lead-id`, indice e tipo; n8n verifica il token di sessione emesso al primo step (o Turnstile) e scrive su Drive | Form + n8n |
...
| Lead non convertito | Regola di età: `_inbox/<lead-id>` più vecchia di 60 giorni viene cancellata (n8n a cron o l'editor all'avvio), coerente col GDPR | n8n / editor |
...
Spazio: con la compressione un lead pesa 2-25 MB. Il VPS non viene toccato, la
quota Drive cresce di pochi GB l'anno anche a 130 lead al mese, e i clienti attivi
restano sotto i 30 MB l'uno (oggi Cavaliere pesa 23 MB con tutti gli artefatti).
...
| Ripresa upload | no (inutile a 0,5 MB) | sì | no (multipart solo sopra 5 MB) | dipende |
...
1. Piano Workspace di info@consulbuild.com e quota Drive disponibile.
...
4. Test su iPhone reale dentro Instagram e Facebook, e su un Android economico,
   con 30 foto su 4G: tempo, memoria, fallimenti.
