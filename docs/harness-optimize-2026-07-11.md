...
| Copy baseline che fallisce il gate anti-slop attuale (6 bloccanti cavaliere, 1 costruzioni) | `out/*/copy.json` pre-gate | **Misurato** con `scripts/regate-copy.mjs` (nuovo, solo report). Nota di onestà: il singolo bloccante di costruzioni («in tutta la» ×3) è borderline — è il piazzamento SEO locale a 3 previsto dalla skill, che il `--consenti` non copre perché il trigramma include la preposizione; più un limite del gate che vero slop. I 6 di cavaliere invece sono reali (copy pre-ciclo anti-ripetizione, mai passato dal critico) |
