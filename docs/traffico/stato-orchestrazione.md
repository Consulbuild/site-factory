...
| T1c | chiuso (`5daed74`…`1e83d6d`; 14 agenti, 2,4 M token). Lighthouse locale 75 → 94. Ripubblicazione di Cavaliere fatta dall'orchestratore (ok di Mattia del 15/09 sull'ottimizzazione da telefono): **PageSpeed dal vivo mobile 79 → 98 (LCP 5,7 → 2,5 s, 3.453 → 487 KiB)**, desktop 98, SEO 100, accessibilità 98. Aperto: preset canon a 88 in locale per i font (158 KB prima del primo disegno) → piano piccolo sui font se un cliente ha canon |
| Test reali T4 (15/09) | campione 384 pagine 0,97 $ (`e94da65`, decisione T4 10) · mappa Cavaliere 0,47 $, 13 target (decisione T4 11) · fatti a mano dall'orchestratore: F1 sede Tally (`edecf5f`), 40101 = guasto da ritentare (`85fdfe3`) |
| «Calibrazione T4» chiusa (`25e391a`, `fa427bb`, `454da4a`, `69b620a`; flusso leggero: 6 agenti, ~1 M token, 40 min): comuni e target entro 25 km, mestieri altrui fuori, 22 domini nazionali (verificati a mano), F2-F4; banco mappa 182/0. Mappa Cavaliere 13 → 16 target tutti entro 20 km (spesa 0,35 $; totale test 1,79 $). Aperti: lessico senza «rifacimento impianto elettrico/idraulico» (≈ 0,09 $), chiavi React duplicate nella timeline di `status-bar.tsx`, elenco domini curato a mano da mantenere. Coda originale: | C-a comuni misurati e target con comune entro 25 km; C-b teste di mestiere altrui non target; C-c dominio in più province = non impresa locale (ricalcolo gratuito del campione dalla cache); ricalcolo mappa Cavaliere (tetto 0,50 $); F2 409 su DELETE cliente e sospensione del Sito col lavoro mappa vivo; F3 focus di `confirm-dialog.tsx` solo all'apertura; F4 chip della status bar a 400 px. Poi test mirati sui concorrenti: Labs ranked keywords + Backlinks (≈ 0,15 $) e C4-C5 (≈ 0,26 $) |
| Notte 16/09 (via libera di Mattia) | in parallelo, scope.json unito dall'orchestratore: **«velocità per tutti i siti»** (run `wf_a22e44cf-5bc`, task `wl56iovms`, flusso leggero) e **G1** (run `wf_aca8b2d9-710`, task `wgktejv9q`, workflow standard, calibrazione senza schede reali, tetto 0,60 $). Poi in ordine: T5a → T5b → T2a → T5c → T2b, poi T7a, T7b, T8, G2, G3. Gli agenti non svuotano scope.json: lo aggiorna l'orchestratore a ogni avvio |
| T5a | piano riallineato (`145cf2a`), dopo «velocità per tutti» (tocca anche la build) |
...
- **Cavaliere ripubblicato col servizio Sito (15/09, ok di Mattia)**: Mattia ha chiesto l'indicizzazione della home; build
  ...
  l'apex con percorso e query; «Always Use HTTPS» attivato da Mattia: tutte le varianti (http, www) arrivano con 301 a
  `https://cavalierebuild.it/` (verificato). **Standard per i clienti futuri** (www + https) da automatizzare in T2a o
