...
(`docs/traffico/piano-T3.md`: `traffico/dati.json`, orari, attestati, comuni ISTAT,
pre-compilazione) e **T4** (`docs/traffico/piano-T4.md`: `lib/dataforseo.ts`, mappa query con
volumi per comune, cache e costi).
...
secondarie scelte dai competitor reali della zona e dai servizi veri del cliente, servizi,
attributi, descrizione, orari, foto, link con UTM verso le pagine del sito. Ogni voce ha la sua
...
- Niente invenzioni: ogni servizio, attributo, orario, frase della descrizione è tracciabile a
  `contesto.json`, `traffico/dati.json` o a una fonte pubblica registrata.
...
- Ditte individuali: nessuna ricerca DataForSEO col nome (decisioni T3, punto 10).
...
1. **Raccolta competitor** per le query principali della mappa (T4) e il comune della sede:
   schede della zona dai risultati Maps/local pack (DataForSEO), con categorie, numero di
   recensioni, valutazione, orari, attributi disponibili nei dati pubblici; cache e costo per
   cliente registrati.
...
5. **Orari, attributi, foto, link**: orari e attributi da `traffico/dati.json` e fonti registrate;
   foto reali del cliente scelte e ordinate con motivo (mai AI); link al sito verso la pagina
   servizio giusta con UTM (ricerca §6.3).
...
- Senza chiavi: «non configurata» leggibile, nessuna eccezione; ditta individuale: nessuna chiamata.
